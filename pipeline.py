...
def get_raw_html(source_type="live", url=LIVE_PORTAL_URL, max_pages=5):
    ...
    if source_type == "live":
        ...
        if not os.path.exists(MOCK_FILE_PATH):
            print(f"[WARNING] {MOCK_FILE_PATH} not found. Generating a baseline skeleton file matching all 7 sectors...")
            skeleton_html = """
            <!DOCTYPE html>
            <html>
            ...
            <body>
                <div id="portal-data">
                    <div class="job-post" data-sector="Agriculture & Food Systems" data-region="Harare">
                        <h3 class="title">Lead Agronomist & Soil Chemist</h3>
                        <p class="skills">Required: Agronomy, GIS Mapping, Drip Irrigation Management, Soil Chemistry Analysis</p>
                    </div>
                    <div class="job-post" data-sector="Medical & Health Sciences" data-region="Bulawayo">
                        <h3 class="title">Clinical Epidemiologist</h3>
                        <p class="skills">Required: Biostatistics, Clinical Trials, Data Management, Public Health Reporting</p>
                    </div>
                    <div class="job-post" data-sector="Engineering & Technology" data-region="Manicaland">
                        <h3 class="title">Renewable Energy Systems Engineer</h3>
                        <p class="skills">Required: Solar PV Design, CAD Drafting, Grid Integration, Battery Storage Systems</p>
                    </div>
                    <div class="job-post" data-sector="Commerce, Business & Finance" data-region="Harare">
                        <h3 class="title">Senior Risk Analyst</h3>
                        <p class="skills">Required: Financial Modeling, Quantitative Analysis, IFRS 9 Compliance, Risk Mitigation</p>
                    </div>
                    <div class="job-post" data-sector="Applied Arts & Humanities" data-region="Matabeleland North">
                        <h3 class="title">Cultural Heritage Conservationist</h3>
                        <p class="skills">Required: Artifact Archiving, Historical Research, Exhibit Design, Community Engagement</p>
                    </div>
                    <div class="job-post" data-sector="ICT & Computer Science" data-region="Harare">
                        <h3 class="title">Senior Cloud Infrastructure Architect</h3>
                        <p class="skills">Required: AWS Cloud Architecting, Kubernetes containerization, Terraform Infrastructure-as-Code</p>
                    </div>
                    <div class="job-post" data-sector="Tourism & Hospitality" data-region="Victoria Falls">
                        <h3 class="title">Ecotourism Operations Manager</h3>
                        <p class="skills">Required: Sustainable Tourism Management, Wildlife Conservation Liaison, Lodge Logistics, Guest Relations</p>
                    </div>
                </div>
            </body>
            </html>
...
def parse_extracted_html(raw_html, source_type="live"):
    ...
    if source_type.lower().strip() == "live":
        ...
        for idx, post in enumerate(job_posts, 1):
            ...
            # Get the mock sector tag as well to provide context to Gemini
            sector = post.get('data-sector', 'Uncategorized')
            scraped_data_block.append(f"ENTRY #{idx}: SECTOR: {sector} | TITLE: {title} | SKILLS NEEDED: {skills.strip()}")
